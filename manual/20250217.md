...
- window.alert()라고 하지않고 alert()이라고 함수를 호출할 수 있었던 이유는, JS는 상위 스코프로 올라가면서 같은 이름의 함수를 탐색하고, 결국은 가장 상위에 위치한 window에서 alert() 함수를 찾아내서 실행하게 된 것입니다.
...
## 결론 : 브라우저 관련 주요 속성 등이 JS 명세가 아니라 HTML 명세에 정의된 것으로 엄밀히 말하자면 JS의 범위에 들어가있지는 않는데, 정의된 속성을 참조하거나 실행하기 위해서는 JS를 사용해야 하므로 종합적으로 습득해 나가는 것이 FE 개발자로서 나아가는 방향이라고 할 수 있음.
...
: DOM element를 다루는 방힉을 document.getElementById(), document.getElementsByClassName() 등의 API를 통해서 수업했습니다.
...
복사해서 집어넣고
브라우저 실행후 
ctrl + shift + i 눌러서 개발자 도구 활성화 후
console에 jQuery 입력하고 엔터 누르면
...
저희 수업에서도 $를 계속해서 사용할 예정

### 제이쿼리의 장점 

$의 핵심 기능 : DOM element를 select하는 기능

변수명 = document.getElementById('btn-computer');와 같은식으로작성하던 것을
...
변수명에 $가 붙어있습니다 -> document.getElementById()에서 return 되는 타입은 일반적인 DOM element인 \<div>태그인데 반해 $로 셀렉트한 결과는 "제이쿼리 오브젝트(jQuery Object)'로 return 됩니다.
...
```
이라고 작성했을 경우, id가 'abc'인 element가 없을 경우 null을 return 하면서 오류 발생합니다.

하지만 제이쿼리 오브젝트의 경우에는 셀렉트된 element가 없어도 null이 아니라 제이쿼리 으보젝트의 형태를 그대로 return 하기 때문에
...
- getElementByClassName()을 대체하는 . 형태의 작성 -> 버튼 비할성화 함수 수정

### 점수판 UI 개선

: 딜레이를 적절하게 집어넣어, 안내문구 및 점수판 갱신 부분에 생동감을 주겠습니다.
...
'사라지는 과정'과 '나타나는 과정'이라는 상반된 부분이 겹쳐서 비동기적으로 일어나기 때문에 브라우저에서 우리가 학인하게 되는 결과는 '깜빡거림'으로 나타납니다.
...
-> index.html에서 \<script>태그 내에 집어넣어줘야합니다.

-> 중요한 점은 animateNumber가 $의 플러그인이기 때문에 제이쿼리가 꼭 먼저 선행되어야 한다는 점을 명심해야 합니다.
